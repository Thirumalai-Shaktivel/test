...
static inline var_sym_t* VARSYM(Allocator &al, Location &l,
        ...
        LFortran::AST::symbolType sym, decl_attribute_t* x, expr_t* kind)
{
...
#define VAR_SYM_NAME(name, sym, loc) VARSYM(p.m_a, loc, \
        name2char(name), nullptr, 0, nullptr, 0, nullptr, sym, nullptr, nullptr)
#define VAR_SYM_DIM_EXPR(exp, sym, loc) VARSYM(p.m_a, loc, nullptr, \
        nullptr, 0, nullptr, 0, EXPR(exp), sym, nullptr, nullptr)
#define VAR_SYM_DIM_INIT(name, dim, n_dim, init, sym, loc) VARSYM(p.m_a, loc, \
        name2char(name), dim, n_dim, nullptr, 0, EXPR(init), sym, nullptr, nullptr)
#define VAR_SYM_DIM_INIT_KIND(name, dim, n_dim, kind, init, sym, loc) \
        VARSYM(p.m_a, loc, name2char(name), dim, n_dim, \
        nullptr, 0, EXPR(init), sym, nullptr, EXPR(INTEGER(kind, loc)))
#define VAR_SYM_DIM(name, dim, n_dim, sym, loc) VARSYM(p.m_a, loc, \
        name2char(name), dim, n_dim, nullptr, 0, nullptr, sym, nullptr, nullptr)
#define VAR_SYM_DIM_KIND(name, dim, n_dim, kind, sym, loc) VARSYM(p.m_a, loc, \
        name2char(name), dim, n_dim, nullptr, 0, \
        nullptr, sym, nullptr, EXPR(INTEGER(kind, loc)))
#define VAR_SYM_CODIM(name, codim, n_codim, sym, loc) VARSYM(p.m_a, loc, \
        name2char(name), nullptr, 0, codim, n_codim, \
        nullptr, sym, nullptr, nullptr)
#define VAR_SYM_DIM_CODIM(name, dim, n_dim, codim, n_codim, sym, loc) \
        VARSYM(p.m_a, loc, name2char(name), \
        dim, n_dim, codim, n_codim, nullptr, sym, nullptr, nullptr)
#define VAR_SYM_SPEC(x, sym, loc) VARSYM(p.m_a, loc, \
        nullptr, nullptr, 0, nullptr, 0, nullptr, sym, \
        down_cast<decl_attribute_t>(x), nullptr)
