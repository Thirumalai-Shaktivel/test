...
public:
    ...
    void transform_stmts(Vec<ASR::stmt_t*> &body, size_t n_body, AST::stmt_t **m_body) {
        ...
        for (size_t i=0; i<n_body; i++) {
            ...
            if (tmp != nullptr) {
                ASR::stmt_t* tmp_stmt = LCompilers::ASRUtils::STMT(tmp);
    ...
    void visit_Open(const AST::Open_t& x) {
        ...
        if( x.n_args == 1 ) {
            ...
            a_newunit = LCompilers::ASRUtils::EXPR(tmp);
        }
        for( std::uint32_t i = 0; i < x.n_kwargs; i++ ) {
            ...
            if( m_arg_str == std::string("newunit") ||
                ...
                a_newunit = LCompilers::ASRUtils::EXPR(tmp);
                ASR::ttype_t* a_newunit_type = LCompilers::ASRUtils::expr_type(a_newunit);
                if( ( m_arg_str == std::string("newunit") &&
                ...
                this->visit_expr(*kwarg.m_value);
                a_filename = LCompilers::ASRUtils::EXPR(tmp);
                ASR::ttype_t* a_filename_type = LCompilers::ASRUtils::expr_type(a_filename);
                ...
                this->visit_expr(*kwarg.m_value);
                a_status = LCompilers::ASRUtils::EXPR(tmp);
                ASR::ttype_t* a_status_type = LCompilers::ASRUtils::expr_type(a_status);
    ...
    void visit_Close(const AST::Close_t& x) {
        ...
        if( x.n_args == 1 ) {
            this->visit_expr(*x.m_args[0]);
            a_unit = LCompilers::ASRUtils::EXPR(tmp);
        }
        for( std::uint32_t i = 0; i < x.n_kwargs; i++ ) {
            AST::keyword_t kwarg = x.m_kwargs[i];
            std::string m_arg_str(kwarg.m_arg);
            if( m_arg_str == std::string("unit") ) {
                ...
                this->visit_expr(*kwarg.m_value);
                a_unit = LCompilers::ASRUtils::EXPR(tmp);
                ASR::ttype_t* a_newunit_type = LCompilers::ASRUtils::expr_type(a_unit);
                ...
                this->visit_expr(*kwarg.m_value);
                a_iostat = LCompilers::ASRUtils::EXPR(tmp);
                ASR::ttype_t* a_iostat_type = LCompilers::ASRUtils::expr_type(a_iostat);
                ...
                this->visit_expr(*kwarg.m_value);
                a_iomsg = LCompilers::ASRUtils::EXPR(tmp);
                ASR::ttype_t* a_iomsg_type = LCompilers::ASRUtils::expr_type(a_iomsg);
                ...
                this->visit_expr(*kwarg.m_value);
                a_err = LCompilers::ASRUtils::EXPR(tmp);
            }
        }
        if( a_unit == nullptr ) {
            throw SemanticError("`newunit` or `unit` must be specified either in argument or keyword arguments.",
    ...
    void create_read_write_ASR_node(const AST::stmt_t& read_write_stmt, AST::stmtType _type) {
        ...
        for( std::uint32_t i = 0; i < n_args; i++ ) {
            if( m_args[i].m_value != nullptr ) {
                ...
                *args[i] = LCompilers::ASRUtils::EXPR(tmp);
            }
        }
        for( std::uint32_t i = 0; i < n_kwargs; i++ ) {
            ...
            if( m_arg_str == std::string("unit") ) {
                ...
                if (kwarg.m_value != nullptr) {
                    this->visit_expr(*kwarg.m_value);
                    a_unit = LCompilers::ASRUtils::EXPR(tmp);
                    ASR::ttype_t* a_unit_type = LCompilers::ASRUtils::expr_type(a_unit);
                ...
                this->visit_expr(*kwarg.m_value);
                a_id = LCompilers::ASRUtils::EXPR(tmp);
                ASR::ttype_t* a_status_type = LCompilers::ASRUtils::expr_type(a_id);
                if (!ASR::is_a<ASR::Character_t>(*ASRUtils::type_get_past_pointer(a_status_type))) {
                ...
                if (kwarg.m_value != nullptr) {
                    ...
                    a_fmt = LCompilers::ASRUtils::EXPR(tmp);
                    ASR::ttype_t* a_fmt_type = LCompilers::ASRUtils::expr_type(a_fmt);
        ...
        for( std::uint32_t i = 0; i < n_values; i++ ) {
            ...
            a_values_vec.push_back(al, LCompilers::ASRUtils::EXPR(tmp));
        }
        if( _type == AST::stmtType::Write ) {
    ...
    void visit_Associate(const AST::Associate_t& x) {
        ...
        ASR::expr_t* target = LCompilers::ASRUtils::EXPR(tmp);
        this->visit_expr(*(x.m_value));
        ASR::expr_t* value = LCompilers::ASRUtils::EXPR(tmp);
    ...
    void visit_AssociateBlock(const AST::AssociateBlock_t& x) {
        ...
        for( size_t i = 0; i < x.n_syms; i++ ) {
            ...
            ASR::expr_t* tmp_expr = LCompilers::ASRUtils::EXPR(tmp);
    ...
    void visit_Allocate(const AST::Allocate_t& x) {
        ...
        for( size_t i = 0; i < x.n_args; i++ ) {
            ...
            ASR::expr_t* tmp_stmt = LCompilers::ASRUtils::EXPR(tmp);
        ...
        if( x.n_keywords >= 1 ) {
            ...
            if( stat_cond ) {
                ...
                stat = LCompilers::ASRUtils::EXPR(tmp);
            } else if( errmsg_cond ) {
                this->visit_expr(*(x.m_keywords[0].m_value));
                errmsg = LCompilers::ASRUtils::EXPR(tmp);
            } else if( source_cond ) {
                this->visit_expr(*(x.m_keywords[0].m_value));
                source = LCompilers::ASRUtils::EXPR(tmp);
            }
        }
...
// otherwise does nothing.
    ASR::stmt_t* create_implicit_deallocate(const Location& loc) {
        ...
        for( auto& item: current_scope->get_scope() ) {
            if( item.second->type == ASR::symbolType::Variable ) {
                const ASR::symbol_t* sym = LCompilers::ASRUtils::symbol_get_past_external(item.second);
        ...
        return LCompilers::ASRUtils::STMT(ASR::make_ImplicitDeallocate_t(al, loc,
    ...
    void visit_Deallocate(const AST::Deallocate_t& x) {
        ...
        for( size_t i = 0; i < x.n_args; i++ ) {
            ...
            if( tmp_expr->type != ASR::exprType::Var ) {
                ...
                if( LCompilers::ASRUtils::symbol_get_past_external(tmp_sym)->type != ASR::symbolType::Variable ) {
                    throw SemanticError("Only an allocatable variable symbol "
                                        "can be deallocated.",
                                        tmp_expr->base.loc);
                } else {
            ...
            case AST::case_stmtType::CaseStmt: {
                ...
                if (AST::is_a<AST::CaseCondExpr_t>(*(Case_Stmt->m_test[0]))) {
                    ...
                    for( std::uint32_t i = 0; i < Case_Stmt->n_test; i++ ) {
                        ...
                        a_test_vec.push_back(al, LCompilers::ASRUtils::EXPR(tmp));
                    }
                    ...
                    if( condrange->m_start != nullptr ) {
                        ...
                        if( LCompilers::ASRUtils::expr_type(m_start)->type != ASR::ttypeType::Integer ) {
                    ...
                    if( condrange->m_end != nullptr ) {
                        ...
                        if( LCompilers::ASRUtils::expr_type(m_end)->type != ASR::ttypeType::Integer ) {
                            throw SemanticError(R"""(Expression in Case selector can only be an Integer)""",
                                                x.base.loc);
                        }
                    }
    ...
    void visit_Select(const AST::Select_t& x) {
        ...
        ASR::expr_t* a_test = LCompilers::ASRUtils::EXPR(tmp);
    ...
    ASR::stmt_t* create_implicit_deallocate_subrout_call(ASR::stmt_t* x) {
        ...
        const ASR::symbol_t* subrout_sym = LCompilers::ASRUtils::symbol_get_past_external(subrout_call->m_name);
        ...
        for( size_t i = 0; i < subrout_call->n_args; i++ ) {
            if( subrout_call->m_args[i].m_value &&
                ...
                const ASR::symbol_t* sym = LCompilers::ASRUtils::symbol_get_past_external(arg_var->m_v);
                if( sym->type == ASR::symbolType::Variable ) {
                    ASR::Variable_t* var = ASR::down_cast<ASR::Variable_t>(sym);
                    const ASR::Var_t* orig_arg_var = ASR::down_cast<ASR::Var_t>(subrout->m_args[i]);
                    const ASR::symbol_t* orig_sym = LCompilers::ASRUtils::symbol_get_past_external(orig_arg_var->m_v);
        ...
        return LCompilers::ASRUtils::STMT(ASR::make_ImplicitDeallocate_t(al, x->base.loc,
                    del_syms.p, del_syms.size()));
    }
    ...
    void visit_Assignment(const AST::Assignment_t &x) {
        ...
        if( LCompilers::ASRUtils::use_overloaded_assignment(target, value,
            current_scope, asr, al, x.base.base.loc,
            [&](const std::string &msg, const Location &loc) { throw SemanticError(msg, loc); }) ) {
            overloaded_stmt = LCompilers::ASRUtils::STMT(asr);
        }
        ASR::ttype_t *target_type = LCompilers::ASRUtils::expr_type(target);
    ...
    void visit_SubroutineCall(const AST::SubroutineCall_t &x) {
        ...
        if (x.n_member == 1) {
            ...
            v_expr = LCompilers::ASRUtils::EXPR(v_var);
        ...
        if (x.n_keywords > 0) {
            ASR::symbol_t* f2 = LCompilers::ASRUtils::symbol_get_past_external(original_sym);
    ...
    void visit_If(const AST::If_t &x) {
        visit_expr(*x.m_test);
        ASR::expr_t *test = LCompilers::ASRUtils::EXPR(tmp);
        Vec<ASR::stmt_t*> body;
        body.reserve(al, x.n_body);
        transform_stmts(body, x.n_body, x.m_body);
    ...
    void visit_ImpliedDoLoop(const AST::ImpliedDoLoop_t& x) {
        ...
        a_start = LCompilers::ASRUtils::EXPR(tmp);
        this->visit_expr(*(x.m_end));
        a_end = LCompilers::ASRUtils::EXPR(tmp);
        if( x.m_increment != nullptr ) {
            this->visit_expr(*(x.m_increment));
            a_increment = LCompilers::ASRUtils::EXPR(tmp);
        }
        ...
        //                                                 ASR::storage_typeType::Default, LCompilers::ASRUtils::expr_type(a_start),
        ...
        tmp = ASR::make_ImpliedDoLoop_t(al, x.base.base.loc, a_values, n_values,
                                            ...
                                            LCompilers::ASRUtils::expr_type(a_start), nullptr);
    }
    ...
    void visit_DoLoop(const AST::DoLoop_t &x) {
        ...
        if (x.m_var) {
            var = LCompilers::ASRUtils::EXPR(resolve_variable(x.base.base.loc, to_lower(x.m_var)));
        }
        if (x.m_start) {
            visit_expr(*x.m_start);
            start = LCompilers::ASRUtils::EXPR(tmp);
        }
        if (x.m_end) {
            visit_expr(*x.m_end);
            end = LCompilers::ASRUtils::EXPR(tmp);
        }

        ASR::expr_t *increment;
        if (x.m_increment) {
            visit_expr(*x.m_increment);
            increment = LCompilers::ASRUtils::EXPR(tmp);
        } else {
            increment = nullptr;
        }
    ...
    void visit_DoConcurrentLoop(const AST::DoConcurrentLoop_t &x) {
        ...
        ASR::expr_t *var = LCompilers::ASRUtils::EXPR(resolve_variable(x.base.base.loc, to_lower(h.m_var)));
        visit_expr(*h.m_start);
        ASR::expr_t *start = LCompilers::ASRUtils::EXPR(tmp);
        visit_expr(*h.m_end);
        ASR::expr_t *end = LCompilers::ASRUtils::EXPR(tmp);
        ASR::expr_t *increment;
        if (h.m_increment) {
    ...
    void visit_ForAllSingle(const AST::ForAllSingle_t &x) {
        ...
        ASR::expr_t *var = LCompilers::ASRUtils::EXPR(
        ...
        assign_stmt = LCompilers::ASRUtils::STMT(tmp);
        ASR::do_loop_head_t head;
        head.m_v = var;
        head.m_start = start;
        head.m_end = end;
    ...
    void visit_Stop(const AST::Stop_t &x) {
        ...
        if (x.m_code) {
            visit_expr(*x.m_code);
            code = LCompilers::ASRUtils::EXPR(tmp);
        } else {
            code = nullptr;
        }
    ...
    void visit_Nullify(const AST::Nullify_t &x) {
        ...
        for( size_t i = 0; i < x.n_args; i++ ) {
            ...
            if( tmp_expr->type != ASR::exprType::Var ) {
                ...
                const ASR::Var_t* tmp_var = ASR::down_cast<ASR::Var_t>(tmp_expr);
                ASR::symbol_t* tmp_sym = tmp_var->m_v;
                if( LCompilers::ASRUtils::symbol_get_past_external(tmp_sym)->type
