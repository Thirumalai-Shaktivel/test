...
#define CWRAPPER_END                                                           \
    ...
    catch (LCompilers::LFortranException & e)                                    \
    {                                                                          \
        return e.error_code();                                                 \
    }                                                                          \
    catch (...)                                                                \
        ...
        const char *input, lfortran_ast_t **ast)
{
    ...
    LCompilers::AST::ast_t* result;
    LCompilers::diag::Diagnostics diagnostics;
    LCompilers::Result<LCompilers::AST::TranslationUnit_t*> res
        = LCompilers::parse(self->al, input, diagnostics);
    if (res.ok) {
        ...
        char **str)
{
    ...
    std::string p = LCompilers::pickle(ast->m);
