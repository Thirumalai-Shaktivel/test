...
using LCompilers::AST::BaseWalkVisitor;
...
std::vector<int> tokens(Allocator &al, const std::string &input) {
    ...
    auto res = LCompilers::tokens(al, input, diagnostics);
    if (res.ok) {
        return res.result;
    } else {
...
TEST_CASE("Test longer parser (N = 500)") {
    ...
    auto result = LCompilers::TRY(parse(al, text, diagnostics))->m_items[0];
...
#define cast(type, p) (LCompilers::AST::type##_t*) (p)
...
TEST_CASE("Location") {
    ...
    Allocator al(1024*1024);
    LCompilers::diag::Diagnostics diagnostics;
    LCompilers::AST::ast_t* result = LCompilers::TRY(parse(al, input, diagnostics))->m_items[0];
...
TEST_CASE("Errors") {
    ...
    input = "(2+3+";
    Result<LCompilers::AST::TranslationUnit_t*> res = parse(al, input, diagnostics);
    CHECK(res.ok == false);
