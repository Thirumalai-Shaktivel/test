...
{
    LCompilers::AST::TranslationUnit_t* result;
    LCompilers::diag::Diagnostics diagnostics;
    LCompilers::Result<LCompilers::AST::TranslationUnit_t*> res
        = LCompilers::parse(al, s, diagnostics);
    if (res.ok) {
        result = res.result;
    } else {
        throw ParserError0();
    }
    LCompilers::AST::ast_t* ast;
    if (result->n_items >= 1) {
        ast = result->m_items[0];
    } else {
        ast = (LCompilers::AST::ast_t*)result;
    }
    std::string pickle = LCompilers::pickle(*ast);
    std::string src = LCompilers::ast_to_src(*result);
...
TEST_CASE("Multiple units") {
    Allocator al(4*1024);
    LCompilers::diag::Diagnostics diagnostics;
    LCompilers::AST::TranslationUnit_t* results;
    std::string s = R"(x = x+1
        y = z+1)";
    results = LCompilers::TRY(LCompilers::parse(al, s, diagnostics));
    CHECK(results->n_items == 2);
    CHECK(LCompilers::pickle(*results->m_items[0]) == "(= 0 x (+ x 1) ())");
    CHECK(LCompilers::pickle(*results->m_items[1]) == "(= 0 y (+ z 1) ())");

    s = "x = x+1; ; y = z+1";
    results = LCompilers::TRY(LCompilers::parse(al, s, diagnostics));
    CHECK(results->n_items == 2);
    CHECK(LCompilers::pickle(*results->m_items[0]) == "(= 0 x (+ x 1) (TriviaNode [] [(Semicolon) (Semicolon)]))");
    CHECK(LCompilers::pickle(*results->m_items[1]) == "(= 0 y (+ z 1) ())");
    ...
    results = LCompilers::TRY(LCompilers::parse(al, s, diagnostics));
    CHECK(results->n_items == 2);
    CHECK(LCompilers::pickle(*results->m_items[0]) == "(= 0 x (+ x 1) (TriviaNode [] [(Semicolon) (EndOfLine) (EndOfLine) (Semicolon)]))");
    CHECK(LCompilers::pickle(*results->m_items[1]) == "(= 0 y (+ z 1) ())");
    ...
    CHECK(LCompilers::pickle(*results->m_items[0]) == "(+ x 1)");
    CHECK(LCompilers::pickle(*results->m_items[1]) == "(= 0 y (+ z 1) ())");
    CHECK(LCompilers::pickle(*results->m_items[2]) == "a");
    ...
    CHECK(LCompilers::pickle(*results->m_items[0]) == "(Function g [] [] () () () [] [] [] [] [(= 0 x y ()) (= 0 x (* 2 y) ())] [])");
    CHECK(LCompilers::pickle(*results->m_items[1]) == "(= 0 s x ())");
    CHECK(LCompilers::pickle(*results->m_items[2]) == "(= 0 y (+ z 1) ())");
    CHECK(LCompilers::pickle(*results->m_items[3]) == "a");
}
