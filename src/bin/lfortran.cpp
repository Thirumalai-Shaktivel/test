...
std::string get_kokkos_dir()
{
    ...
    throw LCompilers::LFortranException("LFORTRAN_KOKKOS_DIR is not defined");
}
...
int emit_tokens(const std::string &input, std::vector<std::string>
    &tok_strings, std::vector<int> &toks, std::vector<LCompilers::YYSTYPE>
    &stypes)
{
    ...
    auto res = LCompilers::tokens(al, input, diagnostics, &stypes);
    ...
    for (size_t i=0; i < toks.size(); i++) {
        tok_strings.push_back(LCompilers::pickle(toks[i], stypes[i]));
        //std::cout << LCompilers::pickle(toks[i], stypes[i]) << std::endl;
    }
    return 0;
}
...
int prompt(bool verbose)
{
    ...
    while (true) {
        ...
        switch (r.type) {
            ...
            default : throw LCompilers::LFortranException("Return type not supported");
        }
    }
    return 0;
}
...
int emit_prescan(const std::string &infile, CompilerOptions &compiler_options)
{
    ...
    std::string prescan = LCompilers::fix_continuation(input, lm,
...
int emit_tokens(const std::string &infile, bool line_numbers, const CompilerOptions &compiler_options)
{
    ...
    for (size_t i=0; i < toks.size(); i++) {
        std::cout << LCompilers::pickle(toks[i], stypes[i]);
        if (line_numbers) {
...
int emit_ast_f90(const std::string &infile, CompilerOptions &compiler_options)
{
    ...
    LCompilers::diag::Diagnostics diagnostics;
    lm.in_filename = infile;
    LCompilers::Result<LCompilers::AST::TranslationUnit_t*> r
            = fe.get_ast2(input, lm, diagnostics);
    std::cerr << diagnostics.render(input, lm, compiler_options);
    if (r.ok) {
        std::cout << LCompilers::ast_to_src(*r.result,
    ...
    bool indent_unit, CompilerOptions &compiler_options)
{
    ...
    LCompilers::diag::Diagnostics diagnostics;
    lm.in_filename = infile;
    LCompilers::Result<LCompilers::AST::TranslationUnit_t*>
    ...
    std::string source = LCompilers::ast_to_src(*ast, color,
        indent, indent_unit);
    ...
    for (size_t i=0; i < passes.size(); i++) {
        switch (passes[i]) {
            case (ASRPass::do_loops) : {
                LCompilers::pass_replace_do_loops(al, *asr);
                break;
            }
            case (ASRPass::global_stmts) : {
                LCompilers::pass_wrap_global_stmts_into_function(al, *asr, "f");
                break;
            }
            case (ASRPass::implied_do_loops) : {
                LCompilers::pass_replace_implied_do_loops(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::array_op) : {
                LCompilers::pass_replace_array_op(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::flip_sign) : {
                LCompilers::pass_replace_flip_sign(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::fma) : {
                LCompilers::pass_replace_fma(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::loop_unroll) : {
                LCompilers::pass_loop_unroll(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::inline_function_calls) : {
                LCompilers::pass_inline_function_calls(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::dead_code_removal) : {
                LCompilers::pass_dead_code_removal(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::sign_from_value) : {
                LCompilers::pass_replace_sign_from_value(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::div_to_mul) : {
                LCompilers::pass_replace_div_to_mul(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::class_constructor) : {
                LCompilers::pass_replace_class_constructor(al, *asr);
                break;
            }
            case (ASRPass::arr_slice) : {
                LCompilers::pass_replace_arr_slice(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::print_arr) : {
                LCompilers::pass_replace_print_arr(al, *asr, LCompilers::get_runtime_library_dir());
                break;
            }
            case (ASRPass::unused_functions) : {
                LCompilers::pass_unused_functions(al, *asr);
                break;
            }
            default : throw LCompilers::LFortranException("Pass not implemened");
        }
    }
...
int emit_cpp(const std::string &infile, CompilerOptions &compiler_options)
{
    ...
    LCompilers::diag::Diagnostics diagnostics;
    lm.in_filename = infile;
    LCompilers::Result<std::string> cpp = fe.get_cpp(input, lm, diagnostics);
    std::cerr << diagnostics.render(input, lm, compiler_options);
    if (cpp.ok) {
        std::cout << cpp.result;
        return 0;
...
int emit_asm(const std::string &infile, CompilerOptions &compiler_options)
{
    ...
    LCompilers::diag::Diagnostics diagnostics;
    lm.in_filename = infile;
    LCompilers::Result<std::string> r = fe.get_asm(input, lm, diagnostics);
    std::cerr << diagnostics.render(input, lm, compiler_options);
    if (r.ok) {
        std::cout << r.result;
        return 0;
        ...
        CompilerOptions &compiler_options)
{
    ...
    // ASR -> x86 machine code
    {
        ...
        LCompilers::Result<int>
            result = LCompilers::asr_to_x86(*asr, al, outfile, time_report);
        auto t2 = std::chrono::high_resolution_clock::now();
        ...
        CompilerOptions &compiler_options)
{
    ...
    // ASR -> wasm machine code
    {
        ...
        auto t1 = std::chrono::high_resolution_clock::now();
        LCompilers::Result<int>
            result = LCompilers::asr_to_wasm(*asr, al, outfile, time_report);
        auto t2 = std::chrono::high_resolution_clock::now();
        ...
        CompilerOptions &compiler_options)
{
    ...
    if (assembly) {
        throw LCompilers::LFortranException("Not implemented");
    } else {
    ...
    try {
        ...
        */
        ...
        if (kernel) {
            ...
            return LCompilers::run_kernel(arg_kernel_f);
#else
        ...
        if (endswith(arg_file, ".f90")) {
            ...
            if (backend == Backend::llvm) {
                ...
                throw LCompilers::LFortranException("Backend not supported");
            }
            if (err) return err;
