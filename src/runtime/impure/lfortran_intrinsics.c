...
LCOMPILERS_API int64_t _lcompilers_open(char *path, char *flags)
{
    FILE *fd;
    fd = fopen(path, flags);
    if (!fd)
...
LCOMPILERS_API char* _lcompilers_read(int64_t fd, int64_t n)
{
...
LCOMPILERS_API void _lcompilers_close(int64_t fd)
{
    if (fclose((FILE*)fd) != 0)
    {
