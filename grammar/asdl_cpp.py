...
# This is meant for the BaseVisitor which is the parent class of the Body and SymbolTable Visitors
intrinsic_procedures = {
    'kind': ["kind", "selected_int_kind", "selected_real_kind"],
    'array': ["size", "lbound", "ubound", "min", "max", "allocated", "minval", "maxval", "real", "sum", "abs"],
}
...
class ASTVisitorVisitor2(ASDLVisitor):
    ...
    def visitModule(self, mod):
        ...
        self.emit("public:")
        self.emit("  std::map<std::string, std::string> intrinsic_procedures = {")
        for key, functions in intrinsic_procedures.items():
            for function in functions:
                self.emit(fr'      {{"{function}", "lfortran_intrinsic_{key}"}},')
        self.emit("};")
